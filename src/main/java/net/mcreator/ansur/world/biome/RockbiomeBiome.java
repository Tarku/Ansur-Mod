...
import net.mcreator.ansur.block.RockwoolBlock;
...
public class RockbiomeBiome extends AnsurModElements.ModElement {
	...
	private static class BiomeRegisterHandler {
		...
		public void registerBiomes(RegistryEvent.Register<Biome> event) {
			if (biome == null) {
				BiomeAmbience effects = new BiomeAmbience.Builder().setFogColor(-10053121).setWaterColor(-10053121).setWaterFogColor(-10053121)
						.withSkyColor(-10053121).withFoliageColor(-10053121).withGrassColor(-10053121).build();
				BiomeGenerationSettings.Builder biomeGenerationSettings = new BiomeGenerationSettings.Builder()
						.withSurfaceBuilder(SurfaceBuilder.DEFAULT.func_242929_a(new SurfaceBuilderConfig(RockwoolBlock.block.getDefaultState(),
								RockwoolBlock.block.getDefaultState(), RockwoolBlock.block.getDefaultState())));
				biomeGenerationSettings.withStructure(StructureFeatures.STRONGHOLD);
				biomeGenerationSettings.withStructure(StructureFeatures.MINESHAFT);
				biomeGenerationSettings.withStructure(StructureFeatures.PILLAGER_OUTPOST);
				biomeGenerationSettings.withFeature(GenerationStage.Decoration.VEGETAL_DECORATION, Feature.TREE
						.withConfiguration((new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(RockwoodLogBlock.block.getDefaultState()),
								new SimpleBlockStateProvider(RockwoodLeavesBlock.block.getDefaultState()),
				...
				mobSpawnInfo.withSpawner(EntityClassification.CREATURE, new MobSpawnInfo.Spawners(RockwoodRogueEntity.entity, 8, 1, 2));
				biome = new Biome.Builder().precipitation(Biome.RainType.RAIN).category(Biome.Category.NONE).depth(0.3f).scale(2f).temperature(0.8f)
						.downfall(0.3f).setEffects(effects).withMobSpawnSettings(mobSpawnInfo.copy())
						.withGenerationSettings(biomeGenerationSettings.build()).build();
				event.getRegistry().register(biome.setRegistryName("ansur:rockbiome"));
			}
		}
	}
	@Override
	...
	private static class CustomLeaveVineTreeDecorator extends LeaveVineTreeDecorator {
		...
		static {
			...
			tdt.setRegistryName("rockbiome_lvtd");
	...
	private static class CustomTrunkVineTreeDecorator extends TrunkVineTreeDecorator {
		...
		static {
			codec = com.mojang.serialization.Codec.unit(() -> instance);
			tdt = new TreeDecoratorType(codec);
			tdt.setRegistryName("rockbiome_tvtd");
			ForgeRegistries.TREE_DECORATOR_TYPES.register(tdt);
		}
		@Override
		protected TreeDecoratorType<?> func_230380_a_() {
	...
	private static class CustomCocoaTreeDecorator extends CocoaTreeDecorator {
		...
		static {
			codec = com.mojang.serialization.Codec.unit(() -> instance);
			tdt = new TreeDecoratorType(codec);
			tdt.setRegistryName("rockbiome_ctd");
			ForgeRegistries.TREE_DECORATOR_TYPES.register(tdt);
		}
