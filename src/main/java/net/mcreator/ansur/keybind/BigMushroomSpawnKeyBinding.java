...
package net.mcreator.ansur.keybind;
...
import net.mcreator.ansur.procedures.BigMushroomSpawnOnKeyPressedProcedure;
...
public class BigMushroomSpawnKeyBinding extends AnsurModElements.ModElement {
	@OnlyIn(Dist.CLIENT)
	...
	public void initElements() {
		keys = new KeyBinding("key.ansur.big_mushroom_spawn", GLFW.GLFW_KEY_H, "key.categories.misc");
		...
		if (Minecraft.getInstance().currentScreen == null) {
			if (event.getKey() == keys.getKey().getKeyCode()) {
				if (event.getAction() == GLFW.GLFW_PRESS) {
					AnsurMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(0, 0));
		...
		if (type == 0) {
			{
				...
				BigMushroomSpawnOnKeyPressedProcedure.executeProcedure($_dependencies);
			}
		}
	}
}
